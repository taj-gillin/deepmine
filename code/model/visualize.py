...
def visualize_images(model, train_inputs, train_labels_ohe, num_searching=500):
    ...
    rand_idxs = np.random.choice(len(train_inputs), num_searching)

    
    rand_batch = tf.gather(train_inputs, rand_idxs)
    rand_labels = tf.gather(train_labels_ohe, rand_idxs)
    ...
    true_classes = np.argmax(rand_labels, axis=1)

    right_idx = np.where(pred_classes == true_classes)
    wrong_idx = np.where(pred_classes != true_classes)

    right = np.reshape(tf.gather(rand_batch, right_idx), (-1, 16, 16))
    wrong = np.reshape(tf.gather(rand_batch, wrong_idx), (-1, 16, 16))
